...
/*
 * DM Dashboard — V2.28.1 (consolidado)
...
SELECT
    NVL(SUM(CASE
        WHEN CAB.TIPMOV = 'V'
         AND CAB.CODTIPOPER IN (${DM_RULES_SQL.saleTops})
        THEN CAB.VLRNOTA ELSE 0 END), 0) AS FATURAMENTO_BRUTO,

    NVL(SUM(CASE
        WHEN CAB.TIPMOV = 'D'
         AND CAB.CODTIPOPER IN (${DM_RULES_SQL.returnTops})
...
WHERE CAB.STATUSNOTA = 'L'
  ...
  AND CAB.CODEMP IN (${DM_RULES_SQL.companies})
  AND CAB.NUNOTA NOT IN (${DM_RULES_SQL.excludedInvoices})
  AND (
      (CAB.TIPMOV = 'V' AND CAB.CODTIPOPER IN (${DM_RULES_SQL.saleTops}))
      OR
      (CAB.TIPMOV = 'D' AND CAB.CODTIPOPER IN (${DM_RULES_SQL.returnTops}))
  )
` + saleSeller;
    }
...
NFS AS (
    ...
    WHERE C.TIPMOV = 'V'
      AND C.STATUSNOTA = 'L'
      AND C.CODEMP IN (${DM_RULES_SQL.companies})
      AND C.DTNEG >= P.DTINI
      AND C.DTNEG < P.DTFIM + 1
      AND C.CODTIPOPER IN (${DM_RULES_SQL.saleTops})
      AND C.NUNOTA NOT IN (${DM_RULES_SQL.excludedInvoices})
` + nfSeller + `),
RELACOES AS (
    SELECT DISTINCT
        V.NUNOTAORIG,
...
JOIN TGFCAB CAB ON CAB.CODVEND = VEN.CODVEND
WHERE CAB.DTNEG >= ADD_MONTHS(TRUNC(SYSDATE), -18)
  AND CAB.CODTIPOPER IN (${DM_RULES_SQL.financialTops})
  AND VEN.APELIDO IS NOT NULL
ORDER BY VEN.APELIDO`;
    }
...
(function () {
    ...
    var allowedPages = ["overview", "performance", "customers", "brands", "stock", "intelligence"];
    var titles = {
    ...
    function setActivePage(page, persist) {
        ...
        if (page === "intelligence" && window.DMIntelligence && typeof window.DMIntelligence.ensureLoaded === "function") {
            window.DMIntelligence.ensureLoaded();
        }
...
(function () {
  ...
  var EMPRESAS_SQL = DM_RULES_SQL.companies;
...
FATPREV_ATUAL AS (
    ...
    FROM (
        SELECT
            SUM(
                CASE
                    WHEN CAB.CODTIPOPER IN (${DM_RULES_SQL.returnTops}) AND CAB.TIPMOV = 'D'
                        THEN -CAB.VLRNOTA
                    ELSE CAB.VLRNOTA
                END
            ) AS VEND,
...
VENDAS AS (
    SELECT
        SUM(
            CASE
                WHEN CAB.CODTIPOPER IN (${DM_RULES_SQL.returnTops})
                    THEN -CAB.VLRNOTA
                ELSE CAB.VLRNOTA
            END
        ) AS VEND,
        COUNT(DISTINCT
            CASE
                WHEN CAB.CODTIPOPER NOT IN (${DM_RULES_SQL.returnTops})
                    THEN CAB.NUNOTA
            END
        ) AS QTDVEND,
        0 AS OPOR,
